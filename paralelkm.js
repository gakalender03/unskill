...
const CONFIG = {
  ...
  GAS_LIMIT: 300000,
  GAS_PRICE: ethers.parseUnits('0.0000000017', 'ether'),
...
class BridgeManager {
  ...
  async bridgeTokens(wallet, amount, nonce) {
    try {
      const channelId = 2;
  ...
  async processBatch(wallet, startNonce, batchSize, amount) {
    const promises = [];
    for (let i = 0; i < batchSize; i++) {
      promises.push(this.bridgeTokens(wallet, amount, startNonce + i));
    }
...
(async () => {
  try {
    ...
    for (let batch = 1; batch <= totalBatches; batch++) {
      ...
      await bridgeManager.processBatch(wallet, currentNonce, currentBatchSize, amount);

...
const CONFIG = {
  ...
  GAS_PRICE_INCREMENT: ethers.parseUnits('0.0000001', 'gwei'), // 0.0000001 Gwei increment per tx
...
class BridgeManager {
  ...
  async bridgeTokens(wallet, nonceManager, amount, txCount) {
    try {
      ...
      Logger.info(`Tx ${nonce} using gas price: ${parseFloat(gasPriceGwei).toFixed(6)} Gwei`);

      const channelId = 2;
      const timeoutHeight = 0;
      const timeoutTimestamp = BigInt(Math.floor(Date.now() / 1000)) * BigInt(1_000_000_000);
      const salt = ethers.hexlify(ethers.randomBytes(32));

      const instruction = [
        0,
        2,
        ""
      ];
      ...
      if (result && result.success) {
        ...
        this.failedTx++;
        Logger.error(`Tx ${nonce} failed: ${result.error ? result.error.message : 'Unknown error'}`);

        if (result && result.error && result.error.message.includes('nonce too low')) {
